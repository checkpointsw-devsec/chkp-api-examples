...
hosts=$(mgmt_cli -r true -f json show hosts details-level full limit 500)
echo $hosts |  jq -r '.objects[]| select(.name | contains ("Task4")) | [.name, ."ipv4-address", .color] | @csv' | tee add_hosts.csv
echo $hosts |  jq -r '.objects[]| select(.name | contains ("Task4")) | [.name] | @csv' | tee delete_hosts.csv
