...
# uid jimo
# pwd admin
mgmt_cli --root true --format json run-script script-name "Add Gaia user: jimo" script "clish -c 'lock database override'; clish -s -c 'add user jimo uid 0 homedir /home/jimo'; clish -s -c 'set user jimo gid 100 shell /bin/bash'; clish -s -c 'set user jimo password-hash $1$/PAfqAlg$1T9G28s.2jBHq6rFYOdIA1'; clish -s -c 'add rba user jimo roles adminRole'" targets.1 "corporate-gateway" | jq -r '.tasks[]."task-details"[].responseMessage' | base64 --decode -i
